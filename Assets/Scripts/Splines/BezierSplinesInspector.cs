...
public class BezierSplinesInspector : Editor
{
    ...
    private void OnSceneGUI()
    {
        ...
        if (spline != null) handleTransform = spline.transform;
    ...
    private void ShowDirections()
    {
        ...
        Handles.DrawLine(point, point + spline.GetDirections(0f) * DirectionScale);
        int steps = StepsPerCurve * spline.CurveCount;
        for (int i = 1; i <= steps; i++)
        {
            float t = i / (float)steps;
            point = spline.GetPoint(t);
            Handles.DrawLine(point, point + spline.GetDirections(t) * DirectionScale);
        }
    }
    ...
    private Vector3 ShowPoint(int index)
    {
        ...
        if (selectedIndex == index)
        {
            ...
            {
                Undo.RecordObject(spline, $"Move {index}. Point");
    ...
    {
        GUILayout.Label("Seleceted Point");
        EditorGUI.BeginChangeCheck();
